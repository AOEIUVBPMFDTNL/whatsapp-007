...
import com.whatsapp.android.api.message.SendVideoMessageRequest;
 ...
 */
@Service
@ApiType(TypeConstant.TaskType.SEND_VIDEO_MESSAGE)
@RequiredArgsConstructor
public class SendVideoMessageService implements ApiStrategy {
    ...
    private Result execute(User user, String username, String type, MessagePack data, String taskId) {
        ...
        if (downloadBytes == null || downloadBytes.length <= 200) {
            return Result.taskFail(type, taskId, username, StatusResult.fail("视频下载失败"));
        }
        SendMessageResult sendMessageResult = user.sendRequest(new SendVideoMessageRequest(downloadBytes, data));
