...
import com.whatsapp.android.api.message.SendFileMessageRequest;
 ...
 *
 * @author sunnoc
 * @date 2021-03-17 10:21
 */
@Service
@ApiType(TypeConstant.TaskType.SEND_FILE_MESSAGE)
@RequiredArgsConstructor
public class SendFileMessageService implements ApiStrategy {
    ...
    private Result execute(User user, String username, String type, MessagePack data, String taskId) {
        ...
        if (ObjectUtil.isNotNull(fileName) && fileName.endsWith(".txt")) {
            downloadBytes = WhatsAppUtils.modifyTxtMd5(downloadBytes);
        }
        SendMessageResult sendMessageResult = user.sendRequest(new SendFileMessageRequest(downloadBytes, data));
