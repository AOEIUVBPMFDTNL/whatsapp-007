...
import com.whatsapp.android.api.profile.UpdateGroupAddPrivacySettingRequest;
 ...
 */
@Service
@ApiType(TypeConstant.TaskType.UPDATE_GROUP_ADD_PRIVACY_SETTING)
public class UpdateGroupAddPrivacySettingService implements ApiStrategy {
    @Override
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        String username = user.getLoginPack().getUsername();
        UpdatePrivacySettingPack data = dataObject.getObject("data", UpdatePrivacySettingPack.class);
        GetPrivacySettingsResult getPrivacySettingsResult = user.sendRequest(new UpdateGroupAddPrivacySettingRequest(data));
