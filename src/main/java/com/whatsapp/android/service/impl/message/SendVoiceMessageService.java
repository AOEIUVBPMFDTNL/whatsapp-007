...
import com.whatsapp.android.api.message.SendVoiceMessageRequest;
 ...
 */
@Service
@ApiType(TypeConstant.TaskType.SEND_VOICE_MESSAGE)
@RequiredArgsConstructor
public class SendVoiceMessageService implements ApiStrategy {
    ...
    private Result execute(User user, String username, String type, MessagePack data, String taskId) {
        ...
        if (downloadBytes == null || downloadBytes.length <= 200) {
            return Result.taskFail(type, taskId, username, StatusResult.fail("语音下载失败"));
        }
        SendMessageResult sendMessageResult = user.sendRequest(new SendVoiceMessageRequest(downloadBytes, data));
