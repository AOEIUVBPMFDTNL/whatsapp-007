...
import com.whatsapp.android.api.message.SendLocationRequest;
 ...
 *
 * @author sunnoc
 * @date 2021-03-17 10:30
 */
@Service
@ApiType(TypeConstant.TaskType.SEND_LOCATION_MESSAGE)
public class SendLocationService implements ApiStrategy {
    @Resource(name = "tomcatThreadPoolExecutor")
    private Executor tomcatThreadPoolExecutor;

    @Override
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        SendLocationPack data = dataObject.getObject("data", SendLocationPack.class);
    ...
    private Result execute(User user, String username, String type, SendLocationPack data, String taskId) {
        SendMessageResult sendMessageResult = user.sendRequest(new SendLocationRequest(data));
