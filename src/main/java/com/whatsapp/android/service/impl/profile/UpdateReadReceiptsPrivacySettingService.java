...
import com.whatsapp.android.api.profile.UpdateReadReceiptsPrivacySettingRequest;
 ...
 */
@Service
@ApiType(TypeConstant.TaskType.UPDATE_READ_RECEIPTS_PRIVACY_SETTING)
public class UpdateReadReceiptsPrivacySettingService implements ApiStrategy {
    @Override
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        String username = user.getLoginPack().getUsername();
        UpdatePrivacySettingPack data = dataObject.getObject("data", UpdatePrivacySettingPack.class);
        GetPrivacySettingsResult getPrivacySettingsResult = user.sendRequest(new UpdateReadReceiptsPrivacySettingRequest(data));
