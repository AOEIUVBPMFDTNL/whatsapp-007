...
import com.whatsapp.android.api.profile.UpdateCallAddPrivacySettingRequest;
 ...
 */
@Service
@ApiType(TypeConstant.TaskType.UPDATE_CALL_ADD_PRIVACY_SETTING)
public class UpdateCallAddPrivacySettingService implements ApiStrategy {
    @Override
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        String username = user.getLoginPack().getUsername();
        UpdatePrivacySettingPack data = dataObject.getObject("data", UpdatePrivacySettingPack.class);
        GetPrivacySettingsResult getPrivacySettingsResult = user.sendRequest(new UpdateCallAddPrivacySettingRequest(data));
