...
import com.whatsapp.android.api.profile.UpdateOnlinePrivacySettingRequest;
 ...
 */
@Service
@ApiType(TypeConstant.TaskType.UPDATE_ONLINE_PRIVACY_SETTING)
public class UpdateOnlinePrivacySettingService implements ApiStrategy {
    @Override
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        String username = user.getLoginPack().getUsername();
        UpdatePrivacySettingPack data = dataObject.getObject("data", UpdatePrivacySettingPack.class);
        GetPrivacySettingsResult getPrivacySettingsResult = user.sendRequest(new UpdateOnlinePrivacySettingRequest(data));
