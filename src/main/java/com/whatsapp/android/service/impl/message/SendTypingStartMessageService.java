...
import com.whatsapp.android.api.message.SendTypingMessageRequest;
 ...
 *
 * @author sunnoc
 * @date 2021-08-30 10:59
 */
@Service
@ApiType(TypeConstant.TaskType.SEND_TYPING_MESSAGE)
public class SendTypingStartMessageService implements ApiStrategy {
    @Resource(name = "tomcatThreadPoolExecutor")
    private Executor tomcatThreadPoolExecutor;

    @Override
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        TypingMessagePack data = dataObject.getObject("data", TypingMessagePack.class);
    ...
    private Result execute(User user, String username, String type, TypingMessagePack data, String taskId) {
        StatusResult statusResult = user.sendRequest(new SendTypingMessageRequest(data));
