...
import com.whatsapp.android.api.profile.UpdateLastPrivacySettingRequest;
 ...
 */
@Service
@ApiType(TypeConstant.TaskType.UPDATE_LAST_PRIVACY_SETTING)
public class UpdateLastPrivacySettingService implements ApiStrategy {
    @Override
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        String username = user.getLoginPack().getUsername();
        UpdatePrivacySettingPack data = dataObject.getObject("data", UpdatePrivacySettingPack.class);
        GetPrivacySettingsResult getPrivacySettingsResult = user.sendRequest(new UpdateLastPrivacySettingRequest(data));
