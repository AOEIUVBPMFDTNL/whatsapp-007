...
import com.whatsapp.android.api.message.SendResourcesMessageRequest;
 ...
 *
 * @author sunnoc
 * @date 2021-04-27 11:12
 */
@Service
@ApiType(TypeConstant.TaskType.SEND_RESOURCES_MESSAGE)
@RequiredArgsConstructor
public class SendResourcesMessageService implements ApiStrategy {
    ...
    public Result execute(String type, JSONObject dataObject, String taskId, User user) {
        SendResourcesMessagePack data = dataObject.getObject("data", SendResourcesMessagePack.class);
    ...
    private Result execute(User user, String username, String type, SendResourcesMessagePack data, String taskId) {
        String content = apiService.downloadResources(data.getContent(), 60 * 1000);
        if (StringUtils.isEmpty(content)) {
            return Result.taskFail(type, taskId, username, StatusResult.fail("文件下载失败"));
        }
        data.setContent(content);
        SendMessageResult sendMessageResult = user.sendRequest(new SendResourcesMessageRequest(data));
