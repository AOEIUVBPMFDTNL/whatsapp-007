...
public class TaskNotify {
    private ConcurrentMap<CountDownLatch, ProtocolTreeNode> record = new ConcurrentHashMap<>();
    @Getter
    private ConcurrentMap<String, CountDownLatch> recordObject = new ConcurrentHashMap<>();
    private ConcurrentMap<String, Consumer<ProtocolTreeNode>> consumerObject = new ConcurrentHashMap<>();
     ...
     * @param taskId taskId
     */
    public void createEvent(String taskId) {
        CountDownLatch countDownLatch = new CountDownLatch(1);
        record.put(countDownLatch, new ProtocolTreeNode(""));
        recordObject.put(taskId, countDownLatch);
    }
     ...
     * @param consumer 消费者
     */
    public void addConsumer(String taskId, Consumer<ProtocolTreeNode> consumer) {
     ...
     * @param content 通知内容
     */
    public void setEventContent(String taskId, ProtocolTreeNode content) {
        CountDownLatch countDownLatch = recordObject.get(taskId);
        if (countDownLatch == null) {
            return;
        }
     ...
     * @return 事件内容
     */
    public ProtocolTreeNode getEventContent(String taskId) {
     ...
     * @return 事件内容
     */
    public ProtocolTreeNode getEventContent(String taskId, long timeout) {
     ...
     * @param taskId 事件id
     */
    public void closeEvent(String taskId) {
        CountDownLatch countDownLatch = recordObject.get(taskId);
     ...
     */
    private void closeEvent(String taskId, CountDownLatch countDownLatch) {
        record.remove(countDownLatch);
        recordObject.remove(taskId);
        consumerObject.remove(taskId);
    }
