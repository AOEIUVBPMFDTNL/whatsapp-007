...
public class CronUtil {
     ...
     * 开始，非守护线程模式
     */
    public static void start(StandardThreadExecutor threadExecutor) {
        start(false, threadExecutor);
    }
     ...
     */
    synchronized public static void start(boolean isDaemon, StandardThreadExecutor threadExecutor) {
        ...
        scheduler.start(isDaemon, threadExecutor);
    }
     ...
     */
    public static void restart(StandardThreadExecutor threadExecutor) {
        lock.lock();
        try {
            if (null != crontabSetting) {
                //重新读取配置文件
        ...
        schedule(crontabSetting);
        //重新启动
        scheduler.start(threadExecutor);
    }
