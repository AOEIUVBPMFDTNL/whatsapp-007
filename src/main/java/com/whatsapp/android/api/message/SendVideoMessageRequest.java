 ...
 *
 * @author sunnoc
 * @date 2021-03-12 14:19
 */
public class SendVideoMessageRequest extends AbstractRequest<SendMessageResult> {
    private byte[] file;
    private String path;
    private MessagePack messagePack;
    private String finalMsgId;

    public SendVideoMessageRequest(byte[] file, MessagePack messagePack) {
        this.file = file;
    ...
    public String funcName() {
        return TypeConstant.TaskType.SEND_VIDEO_MESSAGE;
    }
    ...
    public boolean request() {
        ...
        if (userIds != null && userIds.size() > 0) {
            user.getGorgeousEngine().SendMedia(userIds, path, "video", getTaskId(), null, messagePack.getCaption(), contextInfoBuilder, timeOut() * 1000);
        } else {
            user.getGorgeousEngine().SendMedia(userId, path, "video", getTaskId(), null, messagePack.getCaption(), contextInfoBuilder, timeOut() * 1000, messagePack.getTcToken());
        }
