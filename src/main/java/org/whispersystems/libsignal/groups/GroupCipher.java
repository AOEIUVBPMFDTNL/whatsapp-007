...
public class GroupCipher {
    ...
    public GroupCipher(SenderKeyStore senderKeyStore, SenderKeyName senderKeyId, String username) {
     ...
     */
    public byte[] encrypt(byte[] paddedPlaintext, SenderKeyRecord record) throws NoSessionException, InvalidKeyException {
        try {
            KeyLockUtil.lock(username);
            log.debug("用户: {}, SenderKeyRecord: {}", username, HexUtil.encodeHexStr(record.serialize()));
            ...
            SenderKeyMessage senderKeyMessage = new SenderKeyMessage(senderKeyState.getKeyId(),
                    ...
                    signatureKey);
            // 若不为第一条消息则直接储存
            boolean storeFlag = senderKey.getIteration() > 0;
