...
public class SessionBuilder {
     ...
     * @param remoteAddress The address of the remote user to build a session with.
     */
    public SessionBuilder(SignalProtocolStore store, SignalProtocolAddress remoteAddress, String username) {
...
//            log.warn("We've already setup a session for this V3 message, letting bundled message fall through...");
            return Optional.absent();
        }

        SignedPreKeyRecord signedPreKeyRecord = signedPreKeyStore.loadSignedPreKey(message.getSignedPreKeyId());
        if (signedPreKeyRecord == null) {
            throw new InvalidKeyIdException("无效id");
        }
        ...
        if (message.getPreKeyId().isPresent()) {
            PreKeyRecord preKeyRecord = preKeyStore.loadPreKey(message.getPreKeyId().get());
            if (preKeyRecord == null) {
                throw new InvalidKeyIdException("getPreKeyId fail");
            }
