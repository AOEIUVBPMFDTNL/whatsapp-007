...
public class SessionCipher {
    ...
    public SessionCipher(SessionStore sessionStore, PreKeyStore preKeyStore,
                         ...
                         SignalProtocolAddress remoteAddress, String username) {
        ...
        this.sessionBuilder = new SessionBuilder(sessionStore, preKeyStore, signedPreKeyStore,
                identityKeyStore, remoteAddress, username);
    }

    public SessionCipher(SignalProtocolStore store, SignalProtocolAddress remoteAddress, String username) {
        this(store, store, store, store, remoteAddress, username);
    }
    ...
    public CiphertextMessage fakeSessionEncrypt(byte[] paddedMessage) throws UntrustedIdentityException {
        try {
            KeyLockUtil.lock(username);
            SessionRecord sessionRecord = sessionStore.loadSession(remoteAddress);
            SessionState sessionState = sessionRecord.getSessionState();
    ...
    private byte[] decrypt(SessionState sessionState, SignalMessage ciphertextMessage)
        ...
        MessageKeys messageKeys = getOrCreateMessageKeys(sessionState, theirEphemeral,
                chainKey, counter);
        //解密消息
        byte[] serialize = ciphertextMessage.serialize();
        String ciphertext = HexUtil.encodeHexStr(serialize);
        ciphertextMessage.verifyMac(sessionState.getRemoteIdentityKey(),
                sessionState.getLocalIdentityKey(),
                messageKeys.getMacKey());
        byte[] plaintext = getPlaintext(messageKeys, ciphertextMessage.getBody());
        log.debug("密文：{}，明文: {}", ciphertext, HexUtil.encodeHexStr(plaintext));
